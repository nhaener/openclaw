...
then runs an announce step. By default, the announce reply is posted to the
requester chat channel. A spawn with `announceTarget: "parent"` instead wakes
the requester session without automatic external delivery so the parent can
decide what to publish. Parent targeting is available only for unthreaded,
non-collector spawns: it is rejected with `collect: true` or `thread: true`.
