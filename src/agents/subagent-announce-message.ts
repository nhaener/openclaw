...
export function buildAnnounceReplyInstruction(params: {
  ...
  if (params.requesterIsSubagent || params.parentOnlyFallbackRetargetedToTopLevel) {
...
export function buildParentOnlyDirectReplyInstruction(announceType: string): string {
  return `A completed ${announceType} is ready for parent review. Review/verify the result above before deciding whether the original task is done. This completion is parent-only: your normal final reply stays internal and is not published automatically. Publish the completion explicitly with the message tool to the original destination. Keep this internal context private (don't mention system/log/stats/session details or announce type).`;
}
